...
def spec_adjacency_pairs(spec: Any, prog: list[RoomReq]
    ...
    def pick(cat: str, used: set[str]) -> str | None:
        for c in rt.subtypes_of(cat) + rt.counts_as(cat)[1:]:
    ...
    for adj in getattr(spec, "adjacency", []) or []:
        ...
        if getattr(adj, "kind", "required") == "prohibited":
            forb.append((x, y))
        elif getattr(adj, "relation", "adjacent") in _DOOR_RELATIONS + _TOUCH_RELATIONS:
            req.append((x, y))
            used.update((x, y))
    return req, forb
