...
def _on_segment(px: int, py: int, w, tol: int = 30) -> bool:
...
def _collinear(a, b, tol: int = 30) -> bool:
    return _on_segment(a.start.x, a.start.y, b, tol) and \
        _on_segment(a.end.x, a.end.y, b, tol)


def _drag_attached(plan: Plan, w, dx: int, dy: int) -> int:
    ...
    for o in plan.walls:
        if o is w or _collinear(o, w):
            continue
        if _on_segment(o.start.x, o.start.y, w):
            attached.append((o, "start"))
        if _on_segment(o.end.x, o.end.y, w):
...
def apply_geometry_ops(plan: Plan, ops: list[Any]) -> ApplyResult:
    ...
    for op in ops:
        ...
        if name == "update_wall":
            ...
            moved = _drag_attached(out, w, dx, dy)
