...
## Found by probing, isolated run (`--fresh-document`, 2 examples x 15 probes)

- BUG (fixed) **`Document.from_plan` never gave rooms anchors**, so a plan straight out of the solver had zero of them and lost all seven room names on its first wall edit — reconciliation fell through to matching by exact wall set, which a wall move invalidates by definition. Adoption now derives faces and ensures anchors at every boundary (`Document._adopt`), deliberately not on the storage-load path where mutating would break `verify_log`.
- BUG **All 11 programme-layer commands are offered to the agent and none are implemented.** `add_room`, `remove_room`, `set_room_area`, `set_room_aspect`, `set_room_zone`, `set_room_priority`, `set_adjacency`, `remove_adjacency`, `set_entrance`, `set_wet_grouping`, `set_storeys` validate cleanly and then fail with "recognised but not implemented". Wiring them needs a `DesignSpec` on the document plus a re-solve path — `loop.repair` exists, nothing connects it to a `Document`. Now declared in `apply.KNOWN_UNIMPLEMENTED` and asserted, so it cannot be forgotten again.
- BUG (harness, fixed) `unimplemented()` subtracted `SPEC_OPS`, so the "every command has a handler" test passed while a third of the vocabulary did nothing. A completeness check that excludes the incomplete part is worse than no check.
- BUG (harness, fixed) The three spec-level probes returned `True` unconditionally — "geometry unchanged by design" — so they reported PASS against commands that did nothing at all. A probe that cannot fail proves nothing.
- BUG (harness) `--fresh-document` without `--fresh-conversation` shows the agent a document that contradicts its own transcript; it noticed ("the document has reverted again") and wasted a turn on it. The two flags should be coupled.
